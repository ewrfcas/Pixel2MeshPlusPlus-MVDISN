...
def reduce_std(tensor, dim):
    mean_ = torch.mean(tensor, dim=dim, keepdim=True)
    devs_squared = torch.square(tensor - mean_)
    reduce_var = torch.mean(devs_squared, dim=dim)
    return torch.sqrt(1e-6 + reduce_var)