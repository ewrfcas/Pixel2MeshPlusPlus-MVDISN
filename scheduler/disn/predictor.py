...
class DISNPredictor(Predictor):
    ...
    def predict_step(self, input_batch):
        ...
        for b in range(batch_size):
            ...
            # self.options.model.disn.resolution
            ...
            # x_ = np.linspace(-1, 1, num=self.options.model.disn.resolution)
            # y_ = np.linspace(-1, 1, num=self.options.model.disn.resolution)
            # z_ = np.linspace(-1, 1, num=self.options.model.disn.resolution)
        ...
        # [split_size, batch, num_sample_points, 1]->[B,S,N,1]->[B,SN]
