...
def make_mesh_transformed_to_N_views_v2(mesh_batch, p2mR, p2mT):
    res = []
    batch_size = mesh_batch.shape[0]
    # num_views = cams.shape[1]
    num_views = len(p2mT)
    ...
    for i in range(batch_size):
        mesh = mesh_batch[i].detach().cpu().numpy()
        R_ori, T_ori = p2mR[0][i].detach().cpu().numpy(), p2mT[0][i].detach().cpu().numpy()
        mesh_ori = mesh @ np.linalg.inv(R_ori.T) + T_ori
        mesh_N_views = []
        for v in range(num_views):
            R_v, T_v = p2mR[v][i].detach().cpu().numpy(), p2mT[v][i].detach().cpu().numpy()
            mesh_v = np.dot(mesh_ori - T_v, R_v.T)
            mesh_N_views.append(torch.tensor(mesh_v, dtype=torch.float32, device=mesh_batch.device))
        mesh_N_views = torch.stack(mesh_N_views, 1)
        res.append(mesh_N_views)
...
class P2MPPModel(nn.Module):
    ...
    def forward(self, input_batch):
        ...
        # vertex_visibility_list = torch.split(vertex_visibility_map_packed, [x.shape[0] for x in ori_mesh.verts_list()],
        #                                      dim=1)
        ...
        # B, N, V, 3
        mesh_all_view = make_mesh_transformed_to_N_views_v2(mesh, p2mR=input_batch['p2mR'], p2mT=input_batch['p2mT'])
        # B, N, S, V, 3
        # TODO 这里并没有处理不同数量的mesh顶点
        all_verts_to_sample = self.ellipsoid.sample_coord[None, None, :, None, :].to(device) \
                              + mesh_all_view[:, :, None, :, :]
        ...
        for v in range(views_num):
            ...
            hypothesis_feature_v = hypothesis_flatten_feature_v.reshape(batch_size, -1, self.htpo_num,
                                                                        self.features_dim)
            hypothesis_feature_only_img_feat_v = hypothesis_feature_v[..., 3:]
            # hypothesis_feature_only_img_feat_v = hypothesis_feature_only_img_feat_v * \
            #                                      vertex_visibility_list[..., v, None, None]
        ...
        # mean_hypothesis_only_im_feat = torch.sum(stacked_hypothesis_only_im_feat, 4) / vertex_visibility_list.sum(-1)[:,
        #                                                                                :, None, None].clamp(min=1)
        ...
        summary_dx1 = torch.norm(masked_dx1, dim=-1).mean()
        ...
        mean_hypothesis_only_im_feat2 = torch.mean(stacked_hypothesis_only_im_feat2, 4)
